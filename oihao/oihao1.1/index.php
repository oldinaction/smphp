...
require __DIR__.'/news/163.php'; //导入采集新闻文件
...
<head>
	...
	<title>OI主页 - 收录独立博客的主页</title>
    <meta name="keywords" content="OI主页,我爱主页,博客大全,网址大全" />
	<meta name="description" content="OI主页，收录独立博客站点，为IT人提供学习交流的场所 —— 我爱主页！" />
...
<div class="f-l">
    <div id="news">
        <h2>今日热点</h2>
        <?php
            $_RS_data = $_SESSION['RS_data'];
            $_show_data = $_RS_data['hot_24'];//只显示24小时点击热点
            $_href_data = $_show_data[0];
            $_text_data = $_show_data[1];
            echo '<ul>';
            for($i=0; $i<5; $i++){
            	echo '<li><a href="'.iconv("gb2312","utf-8//IGNORE",$_href_data[$i]).'" target="_blank">'.iconv("gb2312","utf-8//IGNORE",$_text_data[$i]).'</a></li>';
            }
            echo '</ul>';
        ?>
    </div>
